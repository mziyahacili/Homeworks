...
export default async function Categories({ params }: Props) {
  ...
  const categoryData = data.find(
    (item: NavbarProps) => item.category.toLowerCase() === category.toLowerCase()
  );
  const filteredItems = categoryData ? categoryData.items : [];
  ...
  return (
    <div className="container mt-10 flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
        {filteredItems.map((product: { id: string; title: string; href: string; description: string }) => (
